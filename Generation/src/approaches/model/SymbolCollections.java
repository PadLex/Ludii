...
public class SymbolCollections {
	...
	static HashSet<String> notSoHiddenSymbols = new HashSet<>(Arrays.asList("End", "Graphics", "Phase"));
	...
	public static List<Symbol> filterHidden(List<Symbol> symbols) {
		return symbols.stream().filter(s -> s.usedInGrammar() || notSoHiddenSymbols.contains(s.name())).collect(Collectors.toList());
	}
